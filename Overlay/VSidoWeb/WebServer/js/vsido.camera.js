...
var VSidoCamera = function(config){
	...
	**/
	...
	/**
	* マーカー検出結果画像を表示する。
	* @method viewTextDetect
	* @param {img} 		text 	image要素
    * @example
    * <pre><code>
	* var camera = new VSidoCamera({'ip':'192.168.11.2'});
    * camera.viewTextDetect($('#vsido_camera_text'));	
	* </code></pre>
	**/
	this.viewTextDetect = function(marker,ip) {
		var random = Math.floor(Math.random() * Math.pow(2, 31));
		marker.attr('src', 'http://'+ this.ip +':18085/marker?i=' + Math.random());
	}
